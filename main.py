...
if args.rebuild_lua:
    ...
    with open(lua_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"ToysByFunction.tags = {{ {tag_list_lua} }}\n\n")
        f.write("ToysByFunction.toys = {\n")
        ...
        f.write("ToysByFunction._byItemId = {}\n")
        f.write("ToysByFunction._byTag = {}\n\n")
        f.write("for toyId, data in pairs(ToysByFunction.toys) do\n")
        f.write("    ToysByFunction._byItemId[data.itemId] = toyId\n")
        f.write("    for _, tag in ipairs(data.tags) do\n")
        f.write("        if not ToysByFunction._byTag[tag] then\n")
        f.write("            ToysByFunction._byTag[tag] = {}\n")
        f.write("        end\n")
        f.write("        table.insert(ToysByFunction._byTag[tag], toyId)\n")
        f.write("    end\n")
        f.write("end\n\n")

        f.write("-- Lookup functions\n")
        f.write("--- Returns the toy data table for the given toy ID, or nil.\n")
        f.write("function ToysByFunction:GetByToyId(toyId)\n")
        f.write("    return self.toys[toyId]\n")
        f.write("end\n\n")
        f.write("--- Returns the toy data table for the given item ID, or nil.\n")
        f.write("function ToysByFunction:GetByItemId(itemId)\n")
        ...
        f.write("function ToysByFunction:GetByTag(tag)\n")
